...
def carregar_configuracoes() -> dict:
    """
    Lê o ficheiro de configuração local e extrai credenciais e parâmetros.
    """
    config = {
        "MATRICULA": "",
        "SENHA": "",
            ...
            for linha in f:
                if "=" in linha:
                    ...
                    if chave == "MODO_DESKTOP":
                        config[chave] = valor_limpo.lower() in ["true", "1", "sim", "v"]
                    else:
...
def adicionar_para_reanalise(contrato: str, vendedor_nome: str, vendedor_tel: str,
                             nome_planilha: str, origem: str, dados_completos: dict) -> None:
    """
...
def salvar_historico_concluido(contrato: str, nome_planilha: str, vendedor_nome: str,
                               vendedor_tel: str, status_pag: str) -> None:
    """
...
def conectar_google_sheets(nome_planilha: str, aba: str):
    ...
    utilizando cache local de IDs para otimizar requisições e contornar renomeações.
    """
    scope = [
    ...
    try:
        ...
        print(f"   [ERRO CREDENCIAIS] Falha ao autorizar Google Sheets: {e}")
        return None
    ...
    if not planilha:
        try:
            ...
            print(f"   [AVISO] Planilha '{nome_planilha}' não encontrada no Drive.")
            return None

    try:
        return planilha.worksheet(aba)
    except Exception as e:  # pylint: disable=broad-exception-caught
        print(f"   [AVISO] Aba '{aba}' não encontrada na planilha '{nome_planilha}'. Detalhe: {e}")
        return None
...
def atualizar_planilha_vendedor(sheet, row_csv: dict, dados_site: dict, contrato: str) -> str:
    """
    ...
    dados_cadastrais = [
        str(dados_site["data_venda"]), str(dados_site["nome"]),
        str(dados_site["telefone"]), "", str(row_csv.get("origem", "")),
    ]

    lance_val = 0.00
    try:
        str_lance = str(row_csv.get("lance livre", 0))
        val_limpo = str_lance.replace("R$", "").replace(".", "").replace(",", ".").strip()
        lance_val = float(val_limpo)
    except (ValueError, TypeError):
        pass

    dados_financeiros = [
        dados_site["credito"], lance_val, "", str(contrato),
        str(dados_site["grupo"]), str(dados_site["cota"]),
    ]
...
def atualizar_planilha_geral(sheet, row_csv: dict, dados_site: dict, contrato: str) -> str:
    """
